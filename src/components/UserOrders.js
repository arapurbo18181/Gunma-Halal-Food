      ...
      <div className="overflow-x-auto shadow-md sm:rounded-lg w-full">
        <table className="w-full text-sm text-left text-gray-500 ">
          <thead className="text-xs text-gray-700 bg-gray-50">
            <tr>
              <th scope="col" className="px-1 sm:px-3 md:px-6 py-1 md:py-3">
            ...
            {Orders.map((item, i) => {
              return (
                <tr key={i} className="bg-white border-b hover:bg-gray-50 ">
                  <th
                    scope="row"
                    className="px-1 sm:px-3 md:px-6 py-1 md:py-3 font-medium text-gray-900 whitespace-nowrap "
                  >
                    {item.shipping.state}, {item.shipping.city},{" "}
                    {item.shipping.zip}
                  </th>
                  <td className="px-1 sm:px-3 md:px-6 py-1 md:py-3">
                    {item.total_amount}
                  </td>
                  <td className="px-1 sm:px-3 md:px-6 py-1 md:py-3">
                    {item.due_amount}
                        ...
                        <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                          <table className="w-full text-sm text-left text-gray-500 ">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                              <tr>
                                <th scope="col" className="px-2 md:px-6 py-1 md:py-3">
                                  Product name
                                </th>
                                <th scope="col" className="px-2 md:px-6 py-1 md:py-3">
                                  Quantity
                            ...
                            <tbody>
                              {item.order_items.map((elem, i) => {
                                return (
                                  <tr key={i} className="bg-white border-b hover:bg-gray-50">
                                    <td className="px-6 py-4">{elem.item_product.name}</td>
                                    <td className="px-6 py-4"> {elem.qty} </td>
                                    <td className="px-6 py-4">¥ { Math.round(elem.price)} </td>
                                    <td className="px-6 py-4">{ elem.status == 1 ? "Available" : elem.status == 0 ? "Not Available" : ""} </td>
                                  </tr>
                                );
                              })}
                            </tbody>
